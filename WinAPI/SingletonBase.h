...
- 프로그램을 작성하기 앞서 디자인 타임에서의 다양한 상황에 대해 가이드 및 방향성 제시가 가능하다.
...
- 프로젝트 전체에서 1개만 존재해야 하는 객체나 클래스, 또는 프로젝트 전체에서 공유하는 클래스에 적용할 수 있는 패턴이다.

▷ 장점

- 한번의 키워드(new)를 통해 객체를 생성하며 만약 프로젝트 어느곳에도 사용하지 않으면 인스턴스를 생성하지 않기 때문에 메모리 낭비를 방지할 수 있다.

▷ 단점

- 싱글톤은 대부분 전역 변수로 처리하기 때문에 코드에 대해서 알아내기 어렵고 싱글톤 인스턴스에게 너무 많은 데이터가 공유된다면 유지보수가 매우 어려워 진다.
